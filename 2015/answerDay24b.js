...
function findIdealConfiguration(weights) {
    ...
    for (const group1 of group1Combinations) {
        ...
        for (const group2 of group2Combinations) {
            const remainingWeights2 = remainingWeights.filter(w => !group2.includes(w));
            const group3Combinations = getCombinations(remainingWeights2, targetWeight);
            for (const group3 of group3Combinations) {
                const group4 = remainingWeights2.filter(w => !group3.includes(w));
                if (group4.reduce((sum, w) => sum + w, 0) === targetWeight) {
